...
} from '../../store/favorites/middlware';
...
export const ProductPage = (props) => {
  ...
  const btnInCart = productInCart
    .map((itemCart) => itemCart.product.route)
    .some((item) => item === match.params.route);
  ...
  const toggleSpecificationBtn = () => {
    ...
    return (
      <ShowMore onClick={() => setIsSpecification(!isSpecification)}>
        &#9660;
      </ShowMore>
    );
  };
  ...
  const toggleDimensionsBtn = () => {
    ...
    return (
      <ShowMore onClick={() => setIsDimensions(!isDimensions)}>
        &#9660;
      </ShowMore>
    );
  };
  ...
  const inFavorite = productInFavorite
    .map((item) => item.route)
    .some((item) => item === match.params.route);
          ...
          <>
            ...
            <ContainerDetails>
              ...
              <ContainerProduct>
                ...
                {!inFavorite && (
                  <RegularIconFavorite onClick={() => addToFav(product)} />
                )}
                {inFavorite && (
                  <SolidIconFavorite
                ...
                {screenWidth >= 768 ? (
                  <>
                    ...
                    <Description>
                      Высота - {product.sizes.height} cм,{' '}
                    </Description>
                    <Description>
                      Ширина - {product.sizes.width} cм,{' '}
                    </Description>
                    <Description>
                      Глубина - {product.sizes.length} cм{' '}
                    </Description>
                  </>
                ) : (
                  <>
                ...
                <ActionsContainer>
                  ...
                  <Actions>
                    {btnInCart ? (
                      <Button disabled width={'13rem'} text={'В корзине'} />
                    ) : (
                      <Button
                        width={'13rem'}
                        color={'#7191A6'}
                        text={'Купить'}
                        onClick={() => btnHeandler(product, value)}
                      />
                    )}
                  </Actions>
                </ActionsContainer>
                ...
                <Subtitle>
                  ...
                  {isSpecification && (
                    <SpecificationContainer>
                      <DescriptionKey>Покрытие</DescriptionKey>
                      <Description>
                        {product.specifications.covering}
                      </Description>
                      <DescriptionKey>Обивка</DescriptionKey>
                      <Description>{product.specifications.casing}</Description>
                    </SpecificationContainer>
